...
//TODO doesn't quite work right now... 
pub fn fix_multimers(
    unitig: &unitig::UnitigNode,
    twin_reads: &Vec<types::TwinRead>,
    final_seq: &mut Vec<u8>,
    _args: &cli::Cli,
) {
    ...
    let twin_reads_in_unitig = unitig.read_indices_ori.iter().map(|(rid,_)| &twin_reads[*rid]).collect::<Vec<&TwinRead>>();
    let largest_read = twin_reads_in_unitig.iter().max_by_key(|x| x.base_length).unwrap();

    // Only continue if all reads look like multimers
    if !unitig.is_circular() && twin_reads_in_unitig.iter().any(|x| largest_read.base_length - x.base_length > 200){
        return;
    }

    println!("Potential multimer found for unitig {}", unitig.node_id);

    let start = std::time::Instant::now();
    for read in twin_reads_in_unitig{
        let base_seq = &read.dna_seq;
        for multi in [2] {
            let chopped_sequences = (0..multi).map(|i|{
                &base_seq[i * base_seq.len()/multi..(i+1) * base_seq.len()/multi]
            }).collect::<Vec<&SeqSlice<Dna>>>();

            let min_block_size = 32;
            let max_block_size = 128;
            let gaps = Gaps { open: -2, extend: -1 };

            let template = &chopped_sequences[0];
            let u8_template: Vec<u8> = dna_slice_to_u8(template);
            let r = PaddedBytes::from_bytes::<NucMatrix>(&u8_template, max_block_size);

            let mut all_multimers_match = true;
            for i in 1..multi{
                let query = &chopped_sequences[i];
                let u8_query: Vec<u8> = dna_slice_to_u8(query);
                let q = PaddedBytes::from_bytes::<NucMatrix>(&u8_query, max_block_size);
                let mut a = Block::<true, true>::new(q.len(), r.len(), max_block_size);
                a.align(&q, &r, &NW1, gaps, min_block_size..=max_block_size, 0);
                let res = a.res();
                println!("Multi {} Result: {:?}", multi, res);
                println!("{:?} {:?}", query.to_string(), template.to_string());

                if (res.query_idx as f64) < r.len() as f64 * 0.98 ||
                    (res.reference_idx as f64) < q.len() as f64 * 0.98{
                    all_multimers_match = false;
                }
            }
            if all_multimers_match{
                println!("Found multimer for sequence u{} with multimer size {}", &unitig.node_id, multi);
                let new_seq = final_seq[0..final_seq.len()/multi].to_owned();
                *final_seq = new_seq;
                return;
            }
        }
    }
    println!("Multimer check took {:?}", start.elapsed());
}