...
impl Filter {
    ...
    /// Returns `true` if the element was already in the filter.
    /// If the element was not in the filter, it is inserted.
    pub fn maybe_insert(&self, x: u64) -> bool {
        let mut h = x;
        let mut contained = true;

        for _ in 0..self.hashers{
            h = hash(h);
            let chunk = self.get(h);
            if chunk.load(ORDERING) & 1 << (h % 8) == 0 {
                contained = false;
                chunk.fetch_or(1 << (h % 8), ORDERING);
            }
        }
