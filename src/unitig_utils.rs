...
impl UnitigGraph {
    pub fn get_safe_edges_from_cov_threshold(
        &self,
        initial_edges: &[EdgeIndex],
        ...
        if let Some(safety_cov_edge_ratio_val) = safety_cov_edge_ratio {
            ...
            return normalized_ratios
                .into_iter()
                .enumerate()
                .filter(|x| x.1 < safety_cov_edge_ratio_val)
                .map(|x| initial_edges[x.0])
                .collect::<Vec<_>>();
        } else {
...
pub fn get_base_info_overlaps(
    ...
    for (i, indori) in node.read_indices_ori.iter().enumerate() {
        ...
        if node.read_indices_ori.len() == 1 {
            ...
            let mut end =
                reads[ind].base_length as i64 + 1 - internal_ol_len[0] as i64 - overhangs[0] as i64;
            ...
            range = (
                carryover + overhangs[hang_ind - 1], // TODO
...
pub fn quantile_dist(
    v1: &[([f64; ID_THRESHOLD_ITERS], usize)],
    v2: &[([f64; ID_THRESHOLD_ITERS], usize)],
) -> Option<f64> {
    let quants = [0.25, 0.5, 0.75];
    let quantiles_v1 = quants
        .iter()
        .map(|x| median_weight_multi(v1, *x))
        .collect::<Vec<_>>();
    let quantiles_v2 = quants
        .iter()
        .map(|x| median_weight_multi(v2, *x))
        .collect::<Vec<_>>();
...
pub fn log_distribution_distance(
    v1: &[([f64; ID_THRESHOLD_ITERS], usize)],
    v2: &[([f64; ID_THRESHOLD_ITERS], usize)],
) -> Option<f64> {
    let quants = [0.25, 0.5, 0.75];
    ...
    let quantiles_larger = quants
        .iter()
        .map(|x| median_weight_multi(&larger, *x))
        .collect::<Vec<_>>();
    let quantiles_smaller = quants
        .iter()
        .map(|x| median_weight_multi(&smaller, *x))
        .collect::<Vec<_>>();
    ...
    for i in 0..ID_THRESHOLD_ITERS {
        ...
        let iqr_weight = median_weight(&ratio_distribution, 0.75).unwrap()
            - median_weight(&ratio_distribution, 0.25).unwrap();
        weights.push(iqr_weight + 1.);
    }

    //This is wrong TODO
    let weighted_distance = distances
        .iter()
        .zip(weights.iter())
        .map(|(x, y)| x * y)
        .sum::<f64>();
...
pub fn log_shape_distance(
    ...
    for i in 0..ID_THRESHOLD_ITERS {
        for j in i + 1..ID_THRESHOLD_ITERS {
            ...
            for i in 0..shape_larger.len() {
                shape_distances_dist
                    .push((shape_larger[i] - shape_smaller[i % shape_smaller.len()]).abs());
            }
    ...
    shape_distribution_distance =
        shape_distribution_distances.iter().sum::<f64>() / num_shapes as f64;
...
pub fn log_distribution_distance_new(
    ...
    for i in 0..ID_THRESHOLD_ITERS {
        ...
        if median_larger_3.unwrap()[i] < min_cov_threshold
            && median_smaller_3.unwrap()[i] < min_cov_threshold
            && i != 0
        {
            continue;
        }
    ...
    let weighted_distance = ID_THRESHOLD_ITERS as f64
        * distances
            .iter()
            .zip(weights.iter())
            .map(|(x, y)| x * y)
...
#[inline]
pub fn pseudocount_cov_multi(
    cov1: [f64; ID_THRESHOLD_ITERS],
    cov2: [f64; ID_THRESHOLD_ITERS],
) -> f64 {
...
pub fn rescue_id_est_nanopore_strict(edge: &UnitigEdge, c: u64) -> bool {
    ...
    let max_allowable_diff_snpmers = (edge.overlap.shared_snpmers
        / MAX_ALLOWABLE_SNPMER_ERROR_DIVIDER)
        .max(MAX_ALLOWABLE_SNPMER_ERROR_MISC);
    // 0.05, but increased by 0.01 for each 200 SNPmers.
    let additional_fsv_hetero = (edge.overlap.shared_snpmers as f64
        / MAX_ALLOWABLE_SNPMER_ERROR_DIVIDER as f64)
        / 100.
        / 4.0;
    let additional_fsv_hetero = additional_fsv_hetero.min(0.005);

    if id_est > (1.0 - 0.0050 / 100. - additional_fsv_hetero)
        && edge.overlap.diff_snpmers < max_allowable_diff_snpmers
    {
        return true;
    } else {
        return false;
...
mod tests {
    ...
    fn test_log_dist() {
        ...
        assert!(
            log_distribution_distance(&v1, &v2).unwrap() - 2. * (num / denom).ln().abs() < 0.0001
        );
    }
