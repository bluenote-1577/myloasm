...
impl PipelineTimer {
    ...
    pub fn measure<F: FnOnce() -> R, R>(&mut self, name: &str, f: F) -> R {
        ...
        self.steps.push(StepRecord { name: name.to_string(), duration, ram_gb });
        result
    }
    ...
    pub fn write_tsv(&self, path: &Path) {
        ...
        match File::create(path) {
            Ok(f) => {
                ...
                let _ = writeln!(w, "step\tduration_s\tram_after_gb");
                for s in &self.steps {
                    let ram_str = s.ram_gb.map_or("-".to_string(), |r| format!("{:.2}", r));
                    let _ = writeln!(w, "{}\t{:.3}\t{}", s.name, s.duration.as_secs_f64(), ram_str);
                }
                let _ = writeln!(w, "OTHER\t{:.3}\t-", other.as_secs_f64());
