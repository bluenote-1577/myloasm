...
pub fn dereplicate_with_skani(polished_fasta: &str, args: &Cli) {
    ...
    for (ref_id, query_results) in ani_results.iter() {
        for (query_id, ani_result) in query_results.iter() {
            if ani_result.quant_50_contig_len_r > 500_000.
                && ani_result.quant_50_contig_len_q > 500_000.
            {
                continue;
            }
            ...
            if (ani_result.align_fraction_ref > 0.9 || ani_result.align_fraction_query > 0.9)
                && ani_result.ani > 0.98
            {
                let length_id_query = (
                    ani_result.quant_50_contig_len_q,
                    query_id,
                    &ani_result.query_contig,
                );
                let length_id_ref = (
                    ani_result.quant_50_contig_len_r,
                    ref_id,
                    &ani_result.ref_contig,
                );
                let smaller = if length_id_query < length_id_ref {
                    length_id_query
                } else {
                    length_id_ref
                };
                let larger = if smaller == length_id_query {
                    length_id_ref
                } else {
                    length_id_query
                };
            ...
            if (ani_result.align_fraction_ref > 0.99 || ani_result.align_fraction_query > 0.99)
                && ani_result.ani > 0.999
            {
                let length_id_query = (
                    ani_result.quant_50_contig_len_q,
                    query_id,
                ...
                let mut smaller = if length_id_query < length_id_ref {
                    length_id_query
                } else {
                    length_id_ref
                };
                ...
                if length_id_query.0 / length_id_ref.0 > 0.66
                    && length_id_query.0 / length_id_ref.0 < 1. / 0.66
                {
                    if length_id_query.2.contains(CIRC_STRICT_STRING)
                        && !length_id_ref.2.contains(CIRC_STRICT_STRING)
                    {
                        smaller = length_id_ref;
                    } else if length_id_ref.2.contains(CIRC_STRICT_STRING)
                        && !length_id_query.2.contains(CIRC_STRICT_STRING)
                    {
            ...
            else if (ani_result.align_fraction_ref > 0.90
                || ani_result.align_fraction_query > 0.90)
                && ani_result.ani > args.dereplication_ani / 100.
            {
                let length_id_query = (
                    ani_result.quant_50_contig_len_q,
                    query_id,
                ...
                else if ani_result.align_fraction_query > 0.90
                    && ani_result.align_fraction_ref > 0.90
                    && smaller.0 < 100_000.
                {
                    alternate_indices.insert(smaller.1, (larger.2, ani_result));
                    contig_name_to_id_map.insert(ani_result.query_contig.clone(), query_id);
    ...
    while let Some(record) = fasta_records.next() {
        ...
        if (header.contains(CIRC_STRICT_STRING) || header.contains(CIRC_LAX_STRING))
            && seq.len() > 1_000_000
        {
            num_circular_1m += 1;
        }
        ...
        if let Some(id) = id_opt {
            if let Some(val) = duplicate_indices.get(id) {
                let af = if val.1.align_fraction_query > val.1.align_fraction_ref {
                    val.1.align_fraction_query
                } else {
                    val.1.align_fraction_ref
                };
                let ani = val.1.ani * 100.;
                number_duplicated += 1;
                write!(
                    &mut fasta_writer_duplicated,
                    ">{} secondary_to:|{}| aligned_frac:{:.2} ani:{:.2}\n{}\n",
                    header,
                    val.0,
                    af * 100.,
                    ani,
    ...
    log::info!("Number of contigs >= 1M: {}", num_geq_1000);
    log::info!("Number of contigs >= 100k: {}", num_geq_100);
